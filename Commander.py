...
class FindCyclomaticComp(Resource):     #To find commits and find cyclomatic aomplexities
    ...
    def post(self):     #If Request is Post
        ...
        print("Value of Length_Total_List_CC is ")
        print(Length_Total_List_CC)
        print("Value of self.server.totalCommits-1 is")
        print(self.server.totalCommits-1)

        if Length_Total_List_CC == (self.server.totalCommits-1):
            print("Inside true condition")
            print ("Value of Length_Total_List_CC is ")
            LastTime = time.time() - self.server.timer
